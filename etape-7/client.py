...
alloc_segment = True # ajout de l'option
...
try:
    ...
    if "--debug" in sys.argv:
        debug = True
    if "--no-alloc" in sys.argv: # ajout de l'option
        alloc_segment = False
    host = sys.argv[1]
    port = int(sys.argv[2])
    segname = sys.argv[3]
...
with ControledMemory(RemoteMemory(host, port, segname, segsize, debug, alloc=alloc_segment)) as mem:
